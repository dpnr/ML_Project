...
def svm(r,c):
    ...
    for i in range(0,20):
        train.runtraining_svm(r,c) #from the cross validation I got 0.1 as my optimal value for the learning rate
    ...
    generate.values(train,'data-splits/data.eval.anon',False,"SVM")
    predictionResults = prediction.getprediction(train,'data-splits/data.test',False)
    accuracy = predictionResults['correct']*100.0/(predictionResults['wrong'] + predictionResults['correct'])
